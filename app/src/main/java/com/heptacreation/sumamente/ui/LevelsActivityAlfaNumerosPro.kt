    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val callback = object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                ...
                val intent = Intent(this@LevelsActivityAlfaNumerosPro, GameSelectionActivity::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
                finish()
            }
...
// Fin del código de flecha de regresar del celular
    ...
    private fun setupInfoBar() {
        ...
        val alfaNumerosTxt = alfaText + numerosText
        val spannable      = android.text.SpannableString(alfaNumerosTxt)
        spannable.setSpan(
            android.text.style.ForegroundColorSpan(ContextCompat.getColor(this, R.color.red_primary)),
            0, alfaText.length, android.text.Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
        )
        spannable.setSpan(
            android.text.style.ForegroundColorSpan(ContextCompat.getColor(this, R.color.blue_primary_darker)),
            alfaText.length, alfaNumerosTxt.length, android.text.Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
        )
        tvGameName.text = spannable
    ...
    private fun createLevelButtons() {
        ...
        for (i in levelStrings.indices) {
            val levelLayout = LinearLayout(this).apply {
                ...
                ).apply { bottomMargin = 8.dpToPx(this@LevelsActivityAlfaNumerosPro) }
            ...
            val button = Button(this).apply {
                ...
                if (i < ScoreManager.unlockedLevelsAlfaNumerosPro &&
                    ...
                    setOnClickListener {
                        applyBounceEffect(this) {
                            val locked = if (i < ScoreManager.unlockedLevelsAlfaNumerosPro &&
                                ScoreManager.isLevelBlockedByFailuresAlfaNumerosPro(i + 1))
                                R.string.level_locked_by_failures
                            else R.string.level_locked_message
                            Toast.makeText(this@LevelsActivityAlfaNumerosPro, locked, Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
            ...
            Log.d("LevelsActivityAlfaNumPro", "Added button for level ${i + 1}")
        }
    }
    ...
    private fun showInstructions(level: Int, mode: ResponseModeAlfaNumeros? = null) {
        ...
        mode?.let { intent.putExtra("RESPONSE_MODE", it.name) }
        startActivity(intent)
    }

    private fun showResponseModeDialog(level: Int) {
        val dialog = ResponseModeDialogAlfaNumerosPro(this)
        dialog.setOnResponseModeSelectedListener(object :
            ResponseModeDialogAlfaNumerosPro.OnResponseModeSelectedListenerAlfaNumerosPro {
