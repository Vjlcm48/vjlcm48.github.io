...
class InstructionsActivityPrincipiante : BaseActivity()  {
    ...
    private val timeLimits = mapOf(
        ...
        8 to 26.40, 9 to 26.18, 10 to 25.95, 11 to 26.15, 12 to 25.93, 13 to 25.70, 14 to 25.43,
        15 to 29.54, 16 to 29.02, 17 to 28.69, 18 to 28.36, 19 to 28.03, 20 to 27.70, 21 to 28.12,
        22 to 31.12, 23 to 30.66, 24 to 30.21, 25 to 30.75, 26 to 30.59, 27 to 30.14, 28 to 30.63,
        29 to 30.63, 30 to 30.10, 31 to 31.12, 32 to 30.63, 33 to 30.10, 34 to 29.57, 35 to 29.00,
        36 to 31.56, 37 to 30.96, 38 to 30.36, 39 to 29.76, 40 to 29.16, 41 to 30.44, 42 to 29.79,
        43 to 30.46, 44 to 29.78, 45 to 29.10, 46 to 30.63, 47 to 29.95, 48 to 29.27, 49 to 28.54,
        50 to 28.37, 51 to 30.67, 52 to 29.91, 53 to 29.14, 54 to 28.38, 55 to 27.61, 56 to 29.36,
        57 to 29.71, 58 to 28.85, 59 to 28.00, 60 to 27.14, 61 to 29.23, 62 to 28.38, 63 to 27.47,
        64 to 27.45, 65 to 26.50, 66 to 28.90, 67 to 27.95, 68 to 27.00, 69 to 26.05, 70 to 25.05
    )
    ...
    enableEdgeToEdge()
        ...
        btnChangeMode.setOnClickListener {
            val dialog = ResponseModeDialogPrincipiante(this)
            dialog.setOnResponseModeSelectedListener(object : ResponseModeDialogPrincipiante.OnResponseModeSelectedListener {
                override fun onResponseModeSelected(mode: ResponseMode) {
                    responseMode = mode
                    getSharedPreferences("MyPrefs", MODE_PRIVATE)
                        .edit {
                            putString("selectedResponseModePrincipiante", mode.name)
                        }
                }
            })
            dialog.show()
            ...
            clickAnimatorSet.addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    ...
                    if (mode != null) {
                        ...
                        prefs.edit { putString("selectedResponseModePrincipiante", mode.name) }
                    }
                    val intent = Intent(this@InstructionsActivityPrincipiante, GameActivityPrincipiante::class.java)
                    intent.putExtra("LEVEL", level)
                    if (mode != null) {
                        intent.putExtra("RESPONSE_MODE", mode.name)
                    }
                    intent.putExtra("EXCLUDED_INDEX", excludedIndex)
                    AdManager.showInterstitialOnLevelStart(this@InstructionsActivityPrincipiante, level) {
                        startActivity(intent)
                    }
                }
    ...
    private fun setupInfoBar() {
        ...
        tvScore.text = getString(R.string.score_format, ScoreManager.currentScorePrincipiante)
            ...
            clickAnimatorSet.addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    ...
                    val intent = DifficultySelectionActivity.createIntent(
                        this@InstructionsActivityPrincipiante,
                        "NumerosPlus",
                        true,
                        level,
                        responseMode?.name
