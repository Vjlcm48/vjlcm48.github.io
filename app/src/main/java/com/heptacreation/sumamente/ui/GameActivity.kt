...
class GameActivity : BaseActivity()  {
    ...
    private fun startChronometer() {
        ...
        chronometerTimer = object : CountDownTimer(7000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    elapsedSeconds < 3.0 -> ContextCompat.getColor(this@GameActivity, R.color.green_medium)
                    elapsedSeconds < 5.0 -> ContextCompat.getColor(this@GameActivity, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivity, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivity, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
        ...
        if (isCorrect) {
            ...
            if (attempts >= 2 || (pistaActivada && attempts >= 1)) {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    ...
                    ScoreManager.incrementConsecutiveFailures(currentLevel)
                    Handler(Looper.getMainLooper()).postDelayed({
                        navigateToLevelResult(false)
                    }, 1000)
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        ...
        if (isSuccessful) {
            ScoreManager.resetConsecutiveFailures(currentLevel)
            if (ScoreManager.isLevelBlockedByFailures(currentLevel + 1)) {
                ScoreManager.resetConsecutiveFailures(currentLevel + 1)
            }
        }
