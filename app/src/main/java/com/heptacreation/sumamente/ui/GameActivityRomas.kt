...
class GameActivityRomas : BaseActivity() {
    ...
    private fun generateNumbers() {
        ...
        when (level) {
            ...
            in 29..35 -> {
                smallNumbers.addAll(generateRandomNumbers(17, 1..15))
            }
            in 36..42 -> {
    ...
    private fun startChronometer() {
        ...
        chronometerTimer = object : CountDownTimer(7000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    elapsedSeconds < 3.0 -> ContextCompat.getColor(this@GameActivityRomas, R.color.green_medium)
                    elapsedSeconds < 5.0 -> ContextCompat.getColor(this@GameActivityRomas, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivityRomas, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivityRomas, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        val intent = Intent(this, LevelResultActivityRomas::class.java)
