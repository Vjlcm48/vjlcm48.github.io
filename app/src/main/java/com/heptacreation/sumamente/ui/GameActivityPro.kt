...
class GameActivityPro : BaseActivity()  {
    ...
    private fun generateNumbers() {
        ...
        when (level) {
            in 1..7 -> {
                ...
                largeNumbers.addAll(generateRandomNumbers(3, 7..20))
            }
            in 8..14 -> {
                smallNumbers.addAll(generateRandomNumbers(8, 1..6))
                largeNumbers.addAll(generateRandomNumbers(4, 7..20))
            }
            in 15..21 -> {
                smallNumbers.addAll(generateRandomNumbers(9, 1..6))
                largeNumbers.addAll(generateRandomNumbers(5, 7..22))
            }
            in 22..28 -> {
                smallNumbers.addAll(generateRandomNumbers(10, 1..6))
                largeNumbers.addAll(generateRandomNumbers(6, 7..22))
            }
            in 29..35 -> {
                smallNumbers.addAll(generateRandomNumbers(17, 1..24))
            }
            in 36..42 -> {
                smallNumbers.addAll(generateRandomNumbers(18, 1..25))
            }
            in 43..49 -> {
                smallNumbers.addAll(generateRandomNumbers(19, 1..26))
            }
            in 50..56 -> {
                smallNumbers.addAll(generateRandomNumbers(20, 1..27))
            }
            in 57..63 -> {
                smallNumbers.addAll(generateRandomNumbers(21, 1..28))
            }
            in 64..70 -> {
                smallNumbers.addAll(generateRandomNumbers(22, 1..30))
            }
        }
    ...
    private fun mostrarOverlayPista(saldo: Int) {
        ...
        // Autocierre al agotarse el tiempo
    ...
    private fun startChronometer() {
        ...
        chronometerTimer = object : CountDownTimer(7000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    elapsedSeconds < 3.0 -> ContextCompat.getColor(this@GameActivityPro, R.color.green_medium)
                    elapsedSeconds < 5.0 -> ContextCompat.getColor(this@GameActivityPro, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivityPro, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivityPro, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        val intent = Intent(this, LevelResultActivityPro::class.java)
        ...
        if (isSuccessful) {
            ScoreManager.resetConsecutiveFailuresPro(currentLevel)
            if (ScoreManager.isLevelBlockedByFailuresPro(currentLevel + 1)) {
                ScoreManager.resetConsecutiveFailuresPro(currentLevel + 1)
            }
        }
