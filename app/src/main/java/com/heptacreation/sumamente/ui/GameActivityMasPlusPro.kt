...
class GameActivityMasPlusPro : BaseActivity() {
    ...
    private val elementsList = mutableListOf<MASPlusElementPro>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val responseMode = prefs.getString(
            "selectedResponseModeMasPlusPro",
            intent.getStringExtra("RESPONSE_MODE_MASPLUS")
        )
        useManualAnswer = responseMode == ResponseModeMasPlusPro.TYPE_ANSWER.name
        if (!useManualAnswer) {
        ...
        scoreTextView.text = getString(R.string.score_label, ScoreManager.currentScoreMasPlusPro)
    ...
    private fun generateElementsForLevel(level: Int) {
        elementsList.clear()
        generateMASPlusElementsPro(level)
    }

    private fun generateMASPlusElementsPro(level: Int) {
        ...
        val combinationTypes = mutableListOf<CombinationTypePro>()
        ...
        when (level) {
            ...
            in 22..28 -> {
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.RomanNumber)
                combinationTypes.add(CombinationTypePro.RomanRoman)
                negativeCount = 1
            }
            in 29..35 -> {
                totalElements = 16
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.LetterNumber)
                combinationTypes.add(CombinationTypePro.LetterLetter)
                negativeCount = 2
            }
            in 36..42 -> {
                totalElements = 17
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.NumberDecimal)
                negativeCount = 2
            }
            in 43..49 -> {
                totalElements = 18
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.LetterDecimal)
                negativeCount = 3
            }
            in 50..56 -> {
                totalElements = 19
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.RomanLetter)
                negativeCount = 2
            }
            in 57..63 -> {
                totalElements = 20
                ...
                combinationCount = 2
                combinationTypes.add(CombinationTypePro.RomanDecimal)
                negativeCount = 3
            }
            else -> {
                totalElements = 21
        ...
        val generatedElements = mutableListOf<MASPlusElementPro>()

        repeat(romanCount) {
            val randomInt = Random.nextInt(intRange.first, intRange.last + 1)
            val roman = convertToRoman(randomInt)
            generatedElements.add(MASPlusElementPro(roman, isNegative = false))
        }

        if (letterPool.isNotEmpty()) {
            repeat(letterCount) {
                val letter = letterPool.random()
                generatedElements.add(MASPlusElementPro(letter.toString(), isNegative = false))
            }
        }
        ...
        repeat(decimalCount) {
            ...
            generatedElements.add(MASPlusElementPro(formattedDecimal, isNegative = false))
        }
        ...
        repeat(neededBeforeCombos) {
            val randomInt = Random.nextInt(intRange.first, intRange.last + 1)
            generatedElements.add(MASPlusElementPro(randomInt.toString()))
        }
    ...
    private fun ensureNoConsecutiveDuplicates(list: MutableList<MASPlusElementPro>) {
        ...
        chronometerTimer = object : CountDownTimer(7000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    ...
                    elapsedSeconds < 5.0 -> ContextCompat.getColor(this@GameActivityMasPlusPro, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivityMasPlusPro, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivityMasPlusPro, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        val intent = Intent(this, LevelResultActivityMasPlusPro::class.java)
        ...
        if (isSuccessful) {
            ScoreManager.resetConsecutiveFailuresMasPlusPro(currentLevel)
            if (ScoreManager.isLevelBlockedByFailuresMasPlusPro(currentLevel + 1)) {
                ScoreManager.resetConsecutiveFailuresMasPlusPro(currentLevel + 1)
            }
        }

        else if (attempts >= 2 || (pistaActivada && attempts >= 1)) {
            ScoreManager.incrementConsecutiveFailuresMasPlusPro(currentLevel)
    ...
    private fun generateCombinations(
        ...
        repeat(count) {
            ...
            val comboString = when (comboType) {
                ...
                CombinationTypePro.LetterNumber -> {
                ...
                CombinationTypePro.LetterLetter -> {
            ...
            combos.add(MASPlusElementPro(comboString))
        }
        return combos
    }
    ...
    private fun calculateSum(list: List<MASPlusElementPro>): Int {
        var total = 0.0
        for (elem in list) {
...
data class MASPlusElementPro(
    val value: String,
    val isNegative: Boolean = false
)

enum class CombinationTypePro {
    RomanNumber,
    RomanRoman,
    LetterNumber,
    LetterLetter,
