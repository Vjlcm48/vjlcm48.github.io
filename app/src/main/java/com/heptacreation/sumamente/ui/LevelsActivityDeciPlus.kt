...
class LevelsActivityDeciPlus : BaseActivity()  {
    ...
    enableEdgeToEdge()
        ...
        val callback = object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                ...
                val intent = Intent(this@LevelsActivityDeciPlus, GameSelectionActivity::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
                finish()
            }
...
// Fin del código de flecha de regresar del celular
    ...
    override fun onResume() {
        super.onResume()
        ScoreManager.initDeciPlus(this)
        val maxCompleted = ScoreManager.getMaxLevelForCombo("DeciPlus", "Avanzado")
        if (maxCompleted >= ScoreManager.unlockedLevelsDeciPlus) {
            ScoreManager.unlockedLevelsDeciPlus = maxCompleted + 1
    ...
    private fun createLevelButtons() {
        ...
        for (i in levelStrings.indices) {
            ...
            val button = Button(this).apply {
                ...
                minimumHeight = 56.dpToPx(this@LevelsActivityDeciPlus)
                setPadding(
                    16.dpToPx(this@LevelsActivityDeciPlus),
                    12.dpToPx(this@LevelsActivityDeciPlus),
                    16.dpToPx(this@LevelsActivityDeciPlus),
                    12.dpToPx(this@LevelsActivityDeciPlus)
                )
                ...
                if (i < ScoreManager.unlockedLevelsDeciPlus && !ScoreManager.isLevelBlockedByFailuresDeciPlus(i + 1)) {
                    ...
                    setTextColor(ContextCompat.getColor(this@LevelsActivityDeciPlus, R.color.level_text_unlocked))
            ...
            val lockIcon = ImageView(this).apply {
                layoutParams = LinearLayout.LayoutParams(
                    24.dpToPx(this@LevelsActivityDeciPlus),   // antes: 36.dpToPx(...)
                    24.dpToPx(this@LevelsActivityDeciPlus)
                ).apply {
                    gravity = Gravity.CENTER_VERTICAL
                }
                setImageResource(
