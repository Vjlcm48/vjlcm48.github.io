...
class GameActivityDeciPlusPrincipiante : BaseActivity()  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val responseModeName = sharedPreferencesDeciPlus.getString(
            "selectedResponseModeDialogDeciPlusPrincipiante",
        ...
        scoreTextView.text = getString(R.string.score_label, ScoreManager.currentScoreDeciPlusPrincipiante)
    ...
    private fun mostrarOverlayPista(saldo: Int) {
        ...
        btnUseHint.text = getString(R.string.hint_use_btn)
    ...
    private fun startChronometer() {
        ...
        chronometerTimer = object : CountDownTimer(10000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    elapsedSeconds < 5.0 -> ContextCompat.getColor(this@GameActivityDeciPlusPrincipiante, R.color.green_medium)
                    elapsedSeconds < 8.0 -> ContextCompat.getColor(this@GameActivityDeciPlusPrincipiante, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivityDeciPlusPrincipiante, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivityDeciPlusPrincipiante, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        val intent = Intent(this, LevelResultActivityDeciPlusPrincipiante::class.java)
        intent.putExtra("LEVEL", currentLevel)
        intent.putExtra("IS_SUCCESSFUL", isSuccessful)
        intent.putExtra("ATTEMPTS", attempts)
        intent.putExtra("TIME_SPENT", timeSpentInSeconds)
        intent.putExtra("GAME_MODE", "DeciPlusPrincipiante")
