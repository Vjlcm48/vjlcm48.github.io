    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val callback = object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                ...
                val intent = Intent(this@LevelsActivitySumaRestaPro, GameSelectionActivity::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
                finish()
            }
        ...
        for (i in levelStrings.indices) {
            val levelLayout = LinearLayout(this).apply {
                layoutParams = LinearLayout.LayoutParams(
                    ...
                    bottomMargin = 8.dpToPx(this@LevelsActivitySumaRestaPro)
                }
            ...
            val button = Button(this).apply {
                layoutParams = LinearLayout.LayoutParams(
                    0,
                    48.dpToPx(this@LevelsActivitySumaRestaPro),
                    1f
                ).apply {
                    setMargins(0, 0, 8.dpToPx(this@LevelsActivitySumaRestaPro), 0)
                }
                text = getString(levelStrings[i])
                textSize = 16f
                setTypeface(null, Typeface.BOLD)
                setTextColor(ContextCompat.getColor(this@LevelsActivitySumaRestaPro, android.R.color.white))
                gravity = Gravity.CENTER
                setPadding(
                    16.dpToPx(this@LevelsActivitySumaRestaPro),
                    8.dpToPx(this@LevelsActivitySumaRestaPro),
                    16.dpToPx(this@LevelsActivitySumaRestaPro),
                    8.dpToPx(this@LevelsActivitySumaRestaPro)
                )
                ...
                if (i < ScoreManager.unlockedLevelsSumaRestaPro && (!ScoreManager.isLevelBlockedByFailuresSumaRestaPro(i + 1) || i == 0)) {
                    ...
                    setOnClickListener {
                        if (i < ScoreManager.unlockedLevelsSumaRestaPro && ScoreManager.isLevelBlockedByFailuresSumaRestaPro(i + 1)) {
                            Toast.makeText(
                                this@LevelsActivitySumaRestaPro,
            ...
            val lockIcon = ImageView(this).apply {
                layoutParams = LinearLayout.LayoutParams(
                    36.dpToPx(this@LevelsActivitySumaRestaPro),
                    36.dpToPx(this@LevelsActivitySumaRestaPro)
                ).apply {
                    gravity = Gravity.CENTER_VERTICAL
                }
                setImageResource(if (i < ScoreManager.unlockedLevelsSumaRestaPro && (!ScoreManager.isLevelBlockedByFailuresSumaRestaPro(i + 1) || i == 0)) R.drawable.ic_unlock else R.drawable.ic_lock)
            ...
            Log.d("LevelsActivitySumaRestaPro", "Added button for level ${i + 1}")
        }
    }
