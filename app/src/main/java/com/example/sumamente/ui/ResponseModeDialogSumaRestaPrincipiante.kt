...
enum class ResponseModeSumaRestaPrincipiante {
    SIMPLE_SELECTION,
    TYPE_ANSWER
}

class ResponseModeDialogSumaRestaPrincipiante(context: Context) : AppCompatDialog(context) {

    private var listener: OnResponseModeSelectedListenerSumaRestaPrincipiante? = null
    ...
    private fun setupButtons() {
        ...
        val buttonBackground = GradientDrawable().apply {
            setColor(ContextCompat.getColor(context, R.color.blue_pressed))
            cornerRadius = 30f
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeSumaRestaPrincipiante.SIMPLE_SELECTION)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeSumaRestaPrincipiante.TYPE_ANSWER)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeSumaRestaPrincipiante) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerSumaRestaPrincipiante) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerSumaRestaPrincipiante {
        fun onResponseModeSelected(mode: ResponseModeSumaRestaPrincipiante)
    }
}
