...
enum class ResponseModeGenioPlusPrincipiante {
    SIMPLE_SELECTION,
    TYPE_ANSWER
}

class ResponseModeDialogGenioPlusPrincipiante(context: Context) : AppCompatDialog(context) {

    private var listener: OnResponseModeSelectedListenerGenioPlusPrincipiante? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeGenioPlusPrincipiante.SIMPLE_SELECTION)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeGenioPlusPrincipiante.TYPE_ANSWER)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeGenioPlusPrincipiante) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerGenioPlusPrincipiante) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerGenioPlusPrincipiante {
        fun onResponseModeSelected(mode: ResponseModeGenioPlusPrincipiante)
    }
}
