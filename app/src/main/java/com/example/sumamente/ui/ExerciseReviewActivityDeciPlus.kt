...
class ExerciseReviewActivityDeciPlus : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val numberList = intent.getDoubleArrayExtra("NUMBER_LIST") ?: doubleArrayOf()
        val correctAnswer = intent.getDoubleExtra("CORRECT_ANSWER", 0.0)
        val userResponses = intent.getDoubleArrayExtra("USER_RESPONSES") ?: doubleArrayOf()
    ...
    private fun setupUI(
        numberList: DoubleArray, correctAnswer: Double, userResponses: DoubleArray,
        excludedIndex: Int
    ) {
        ...
        val formattedCorrectAnswer = String.format(Locale.getDefault(), "%.1f", correctAnswer)
        correctAnswerTextView.text = getString(R.string.correct_answer_format_deci, formattedCorrectAnswer)
        ...
        val formattedUserResponses = if (userResponses.size >= 2) {
            String.format(
                Locale.getDefault(),
                "%s, %s",
                String.format(Locale.getDefault(), "%.1f", userResponses[0]),
                String.format(Locale.getDefault(), "%.1f", userResponses[1])
            )
    ...
    private fun displayNumbers(numberList: DoubleArray, excludedIndex: Int) {
        ...
        for (i in numberList.indices) {
            ...
            val formattedNumber = String.format(Locale.getDefault(), "%.1f", number)
            val spannableString = SpannableStringBuilder(formattedNumber)
            ...
            if (i > 0 && abs(number - numberList[i - 1]) < 0.01) {
                spannableString.setSpan(
