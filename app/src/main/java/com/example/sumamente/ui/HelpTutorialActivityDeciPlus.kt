...
class HelpTutorialActivityDeciPlus : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_help_tutorial_deci_plus)
        initViews()
        startSequence()
    ...
    private fun hideModesDialogDeciPlus() {
        val fadeOut = AnimationUtils.loadAnimation(this, R.anim.activity_fade_out)
    ...
    private fun displayNextNumber() {
        ...
        tvNumber.apply {
            textSize = 127.5f
            text = String.format(Locale.getDefault(), "%.1f", number)
            visibility = View.VISIBLE

            when {
                number < 0 -> setTextColor(ContextCompat.getColor(context, R.color.red))
                number == 0.4 && currentNumberIndex == 5 -> setTextColor(ContextCompat.getColor(context, R.color.black))
                number == 0.4 && currentNumberIndex == 6 -> setTextColor(ContextCompat.getColor(context, R.color.yellow))
                else -> setTextColor(ContextCompat.getColor(context, R.color.black))
            }

            val bounce = AnimationUtils.loadAnimation(this@HelpTutorialActivityDeciPlus, R.anim.bounce_in)
                        ...
                        seconds < 3.0 -> {
                            chronometerTextView.setTextColor(
                                ContextCompat.getColor(
                                    this@HelpTutorialActivityDeciPlus,
    ...
    private fun restartTutorial() {
        ...
        val intent = Intent(this, HelpTutorialActivityDeciPlus::class.java)
        startActivity(intent)
        finish()
    }
