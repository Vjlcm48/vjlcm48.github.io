...
class ExerciseReviewActivityMasPlus : AppCompatActivity() {
        ...
        for (i in elementList.indices) {
            ...
            when {
                ...
                element.matches(Regex("\\d+\\.\\d+")) -> {
                    formatDecimalElement(spannableString)
                }
    ...
    private fun formatDecimalElement(spannableString: SpannableStringBuilder) {

        val decimalIndex = spannableString.indexOf(".")
        if (decimalIndex > 0) {
