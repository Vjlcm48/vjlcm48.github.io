...
class LevelResultActivitySumaResta : AppCompatActivity() {
    ...
    private fun handleSuccessScenario() {
        ...
        ScoreManager.levelScoresSumaResta[currentLevel]?.let { previousScore ->
            ScoreManager.currentScoreSumaResta -= previousScore
        }

        ScoreManager.levelScoresSumaResta[currentLevel] = pointsEarned
        ScoreManager.currentScoreSumaResta = max(ScoreManager.currentScoreSumaResta + pointsEarned, 0)

        if (!ScoreManager.hasCompletedLevelSumaResta(currentLevel)) {
