...
enum class ResponseModeMasPlusPrincipiante {
    SIMPLE_SELECTION,
    TYPE_ANSWER
}

class ResponseModeDialogMasPlusPrincipiante(context: Context) : AppCompatDialog(context) {

    private var listener: OnResponseModeSelectedListenerMasPlusPrincipiante? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.response_mode_dialog_mas_plus)
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeMasPlusPrincipiante.SIMPLE_SELECTION)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeMasPlusPrincipiante.TYPE_ANSWER)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeMasPlusPrincipiante) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerMasPlusPrincipiante) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerMasPlusPrincipiante {
        fun onResponseModeSelected(mode: ResponseModeMasPlusPrincipiante)
    }
}