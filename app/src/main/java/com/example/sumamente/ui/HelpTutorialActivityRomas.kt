...
class HelpTutorialActivityRomas : BaseActivity()  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_help_tutorial_romas)
        initViews()
        startSequence()
    ...
    private fun hideModesDialogRomas() {
        val fadeOut = AnimationUtils.loadAnimation(this, R.anim.activity_fade_out)
    ...
    private fun simulateHandInfoIcons() {
        val infoSimple = findViewById<ImageButton>(R.id.info_seleccion_simple_romas)
        val infoType = findViewById<ImageButton>(R.id.info_escribe_respuesta_romas)
    ...
    private fun displayNextNumber() {
        ...
        tvNumber.apply {
            ...
            when {
                ...
                number == "VI" && currentNumberIndex == 6 -> {
                    tvNumber.setTextColor(ContextCompat.getColor(context, R.color.yellow))
                }
                else -> {
            ...
            val bounce = AnimationUtils.loadAnimation(this@HelpTutorialActivityRomas, R.anim.bounce_in)
    ...
    private fun showChronometerAndInput() {
                    ...
                    when {
                        seconds < 3.0 -> {
                            chronometerTextView.setTextColor(
                                ContextCompat.getColor(this@HelpTutorialActivityRomas, R.color.green_medium)
                            )
                        }
                        seconds < 4.36 -> {
                            chronometerTextView.setTextColor(
                                ContextCompat.getColor(this@HelpTutorialActivityRomas, R.color.orange_dark)
                            )
                        }
                        else -> {
                            chronometerTextView.setTextColor(
                                ContextCompat.getColor(this@HelpTutorialActivityRomas, R.color.red)
                            )
                        }
                    }
        ...
        handler.postDelayed({
            applyBounceEffect(etUserAnswer){
                simulateKeyboardPress("1", etUserAnswer)
            }
        }, 1500)

        handler.postDelayed({
            applyBounceEffect(etUserAnswer) {
                simulateKeyboardPress("9", etUserAnswer)
            }
        }, 2000)
    ...
    private fun showCelebration() {
        ...
        handler.postDelayed({
            ...
            finalPointsTextView.text = getString(R.string.formatted_points, 500)
    ...
    private fun showFinalClosingMessage() {
        ...
        handler.postDelayed({
            ...
            btnPlay.text = getString(R.string.jugar)
    ...
    private fun restartTutorial() {
        stopBackgroundMusic()
        val intent = Intent(this, HelpTutorialActivityRomas::class.java)
        startActivity(intent)
        finish()
    }
