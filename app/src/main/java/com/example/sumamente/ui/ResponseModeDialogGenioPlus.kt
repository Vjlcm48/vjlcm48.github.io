...
class ResponseModeDialogGenioPlus(context: Context) : AppCompatDialog(context) {

    private var listener: OnResponseModeSelectedListenerGenioPlus? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeGenioPlus.SIMPLE_SELECTION)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeGenioPlus.TYPE_ANSWER)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeGenioPlus) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerGenioPlus) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerGenioPlus {
        fun onResponseModeSelected(mode: ResponseModeGenioPlus)
    }
}
