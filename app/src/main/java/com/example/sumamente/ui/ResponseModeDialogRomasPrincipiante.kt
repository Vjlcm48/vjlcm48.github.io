...
enum class ResponseModeRomasPrincipiante {
    SIMPLE_SELECTION_ROMAS,
    TYPE_ANSWER_ROMAS
}

class ResponseModeDialogRomasPrincipiante(context: Context) : AppCompatDialog(context) {

    private var listener: OnResponseModeSelectedListenerRomasPrincipiante? = null
    ...
    private fun setupButtons() {
        val btnSeleccionSimple = findViewById<androidx.appcompat.widget.AppCompatButton>(R.id.btn_seleccion_simple_romas)
        val btnEscribeRespuesta = findViewById<androidx.appcompat.widget.AppCompatButton>(R.id.btn_escribe_respuesta_romas)
        val checkSeleccionSimple = findViewById<ImageView>(R.id.check_seleccion_simple_romas)
        val checkEscribeRespuesta = findViewById<ImageView>(R.id.check_escribe_respuesta_romas)
        val infoSeleccionSimple = findViewById<ImageButton>(R.id.info_seleccion_simple_romas)
        val infoEscribeRespuesta = findViewById<ImageButton>(R.id.info_escribe_respuesta_romas)

        val buttonBackground = GradientDrawable().apply {
            setColor(ContextCompat.getColor(context, R.color.green_medium))
            cornerRadius = 30f
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeRomasPrincipiante.SIMPLE_SELECTION_ROMAS)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeRomasPrincipiante.TYPE_ANSWER_ROMAS)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeRomasPrincipiante) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerRomasPrincipiante?) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerRomasPrincipiante {
        fun onResponseModeSelected(mode: ResponseModeRomasPrincipiante)
    }
}
