...
class ResponseModeDialogRomasPrincipiante(context: Context) : AppCompatDialog(context) {
    ...
    private var listener: OnResponseModeSelectedListenerRomas? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSeleccionSimple?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeRomas.SIMPLE_SELECTION_ROMAS)
            }, 1000)
        }
        ...
        btnEscribeRespuesta?.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                sendResponseModeToCaller(ResponseModeRomas.TYPE_ANSWER_ROMAS)
            }, 1000)
        }
    ...
    private fun sendResponseModeToCaller(mode: ResponseModeRomas) {
    ...
    fun setOnResponseModeSelectedListener(listener: OnResponseModeSelectedListenerRomas?) {
        this.listener = listener
    }

    interface OnResponseModeSelectedListenerRomas {
        fun onResponseModeSelected(mode: ResponseModeRomas)
    }
}
