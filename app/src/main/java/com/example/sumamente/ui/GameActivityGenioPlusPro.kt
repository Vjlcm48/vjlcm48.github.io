...
class GameActivityGenioPlusPro : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val responseMode = prefs.getString("selectedResponseModeGenioPlusPro", intent.getStringExtra("RESPONSE_MODE"))

        if (responseMode != null) {
            useManualAnswer = responseMode == ResponseModeGenioPlusPro.TYPE_ANSWER.name
            if (!useManualAnswer) {
    ...
    private fun startChronometer() {
        ...
        chronometerTimer = object : CountDownTimer(7000, 75) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val textColor = when {
                    timeSpentInSeconds < 3.0 -> ContextCompat.getColor(this@GameActivityGenioPlusPro, R.color.green_medium)
                    timeSpentInSeconds < 5.0 -> ContextCompat.getColor(this@GameActivityGenioPlusPro, R.color.orange_dark)
                    else -> ContextCompat.getColor(this@GameActivityGenioPlusPro, R.color.red)
                }
            ...
            override fun onFinish() {
                ...
                spannableString.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(this@GameActivityGenioPlusPro, R.color.red)),
                    0,
                    finalTime.length,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun navigateToLevelResult(isSuccessful: Boolean) {
        val intent = Intent(this, LevelResultActivityGenioPlusPro::class.java)
